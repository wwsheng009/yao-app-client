...
export default class Form<T> extends YaoClient {
    ...
    async Find(id: number, query: YaoQueryParam.QueryParam): Promise<T> {
        return this.client.Process<T>(`yao.form.Find`, this.name, id, query)
    }
    ...
    async Create(record: T): Promise<number> {
        return this.client.Process<number>(`yao.form.Create`, this.name, record)
    }
    ...
    async Update(id: ID, record: T): Promise<null> {
        return this.client.Process<null>(`yao.form.Update`, this.name, id, record)
    }
    /**
    ...
    async Delete(id: ID): Promise<null> {
        return this.client.Process<null>(`yao.form.Delete`, this.name, id)
    }

    async Load(file: string): Promise<Error> {
        return this.client.Process<Error>(`yao.form.Load`, this.name, file)
    }
